...
export function settle(input: {
  ...
  const pool = input.bets.reduce((sum, bet) => sum + bet.amount, 0);
  ...
  const shares = winning.map((bet) => {
    ...
    const share = totalWeight > 0 ? Math.floor((pool * weight) / totalWeight) : 0;
    return { bet, weight, share };
  });
  let dust = pool - shares.reduce((sum, row) => sum + row.share, 0);
  ...
  const payouts = group(
    shares.map((row) => ({
      ...
      profit: row.share - row.bet.amount,
